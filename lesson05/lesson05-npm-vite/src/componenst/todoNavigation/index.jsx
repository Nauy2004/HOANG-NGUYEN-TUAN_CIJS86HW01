...
function TodoNavigation1(props) {
  let onHandlerChange = props.onHandlerChange
  let handlerDelete = props.handlerDelete
  let onClickAddButton = props.onClickAddButton

  let renderAllTask = () => {
    let styleGr = {}
    return props.data.map((itemTask, index) => {
      if (itemTask.status) {
        styleGr = {    
  ...
  let renderActiveTask = () =>{
    return props.data.map((itemTask) => {
      if (itemTask.status) {
